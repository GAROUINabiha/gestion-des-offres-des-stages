...
$html = file_get_html('https://www.optioncarriere.tn/recherche/emplois?s=informatique&l=');


$divClass = $title='';
$i =0;
foreach($html->find(".job") as $divClass) {
$h = 'https://www.optioncarriere.tn/recherche/emplois?s=informatique&l=';
$answer[$i]['lien'] = $h;
//title
...
//ipl-ratings-bar
foreach($divClass->find(".locations_compact") as $location ) {
$answer[$i]['local'] = $location->plaintext;
$trimmed = trim($location);
}

//description
foreach($divClass->find(".advertise_compact") as $desc) {
...
function array_to_xml1( $answer, $xml_data ) {
...
array_to_xml1($answer,$xml_data);

//saving generated xml file; 
$result = $xml_data->asXML('site2.xml');
...
$xml = simplexml_load_file("site2.xml") or die("Error: Cannot create object");
